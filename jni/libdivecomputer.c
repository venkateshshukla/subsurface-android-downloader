#define FDEBUG(x) fprintf(stderr, "libdivecomputer.c\t:\t%s\n", x)
#include <stdio.h>
#include <unistd.h>
#include <inttypes.h>
#include <string.h>
...
{
	FDEBUG ("dc_status_t create_parser");
...
{
	FDEBUG ("uint32_t calculate_diveid");
	uint32_t csum[5];
...
{
	FDEBUG ("uint32_t calculate_sha1");
	SHA_CTX ctx;
	uint32_t csum[5];
...
{
	FDEBUG ("undo_libdivecomputer_suunto_nr_changes");
...
{
	FDEBUG ("do_libdivecomputer_import");
	dc_status_t rc;
	const char *err;
	FILE *fp = NULL;
