...
dc_status_t
dowork (dc_context_t *context, dc_descriptor_t *descriptor, int fd, const char *rawfile, const char *xmlfile, int memory, int dives, dc_buffer_t *fingerprint)
{
	dc_status_t rc = DC_STATUS_SUCCESS;
	...
	// Open the device.
	message ("Opening the device (%s %s, %d).\n",
		dc_descriptor_get_vendor (descriptor),
		dc_descriptor_get_product (descriptor),
		fd);
	dc_device_t *device = NULL;
	rc = dc_device_open (&device, context, descriptor, fd);
	if (rc != DC_STATUS_SUCCESS) {
